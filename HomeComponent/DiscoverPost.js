...
export default function DiscoverPost({ post }) {
  ...
  return (
    <View style={styles.PostContainer} key={item?._id}>
      <TouchableOpacity style={styles.MainProfileContainerinfeed} onPress={() => navigation.navigate(navigationStrings.UserProfileFeed , {item})}>
        <FastImage source={{ uri: `${user?.profile}` }} style={{ width: 24, height: 24, borderRadius: 500 }} />
        <Text style={styles.inputpostTitle}>{user?.username}</Text>
      </TouchableOpacity>

      <View style={styles.postImageContainer}>
        <Text>{post?.title.slice(0,50)}
        </Text>
        {post?.image !== '' ?
        <FastImage source={{ uri: `${post?.image}` }} style={{ width: "100%" , height:300, borderRadius: 10, marginTop: 10 }} resizeMode={FastImage.resizeMode.contain} />: post.video !== '' ?  
        ""
       :''}
      </View>
      ...
      <View style={styles.ProductContainer}>
        <View style={styles.SubProductContainer}>
          ...
          <View style={{ marginTop: 6 }}>
            <Text style={{ width: "100%", color: "white", marginLeft: 10, fontSize: 13 }}>{product?.title?.slice(0, 34)}</Text>
            <Text style={{ width: "100%", color: "white", marginLeft: 10, fontSize: 11 }}>{product?.desc?.slice(0, 34)}</Text>
          </View>
        </View>
